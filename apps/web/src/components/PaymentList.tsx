...
export default function PaymentList({ currency: _currency }: PaymentListProps) {
    ...
    <div>
      ...
      {searched && !loading && items.length === 0 && (
        <div className="empty-state">
          ...
          <p style={{ fontSize: 13 }}>Try clearing the search or searching by Payment ID / Merchant Reference.</p>
        </div>
      )}
      ...
      {!searched && !loading && (
        <div className="empty-state">
          <p>Use the search above to find Payments by Payment ID or Merchant Reference.</p>
        </div>
      )}
    </div>
  );
